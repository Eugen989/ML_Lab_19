...
new_neuron = tf.keras.models.load_model('model/neuron_weights.keras')
...
def p_lab4():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            print(score)
            return render_template('lab4.html', title="Первый нейрон", menu=menu, class_model=f"Это изображение похоже на {class_names[np.argmax(score)]} с вероятностью {100 * np.max(score)} процентов.")
